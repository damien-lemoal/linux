...
static void blk_zone_active_wplugs_work(struct work_struct *work)
{
	...
	if (blk_queue_enter(disk->queue, BLK_MQ_REQ_NOWAIT))
		return;
					  ...
					  struct bio *bio)
{
	 ...
	 * Splitting guarantees that the parts of a split BIO are passed in
	 * order, so we normally do not need sorting the list of BIOs. However,
	 * when the BIO work unplug and issue BIOs using
	 * submit_bio_noacct_nocheck(), the disk queue limits may have changed
	 * resulting in the plugging of BIO parts which must come before all
	 * BIOs already plugged. So sorting the BIOs when adding them to the
	 * list is needed. This is however a rare case, so we optimize for the
	 * regular add-at-tail case first.
	 */
