 ...
 *
 * The zone write plug will be locked on return.
 */
				    ...
				    struct blk_zone_wplug *zwplug)
{
	...
	if (test_and_set_bit(BLK_ZONE_WPLUG_ACTIVE, &zwplug->flags)) {
 ...
 * If @activate is true and we have a write plug, the write plug is locked
 * and activated before returning.
 */
...
{
	return blk_zone_lookup_wplug(bio, false) != NULL;
}
...
{
	struct blk_zone_wplug *zwplug = blk_zone_lookup_wplug(bio, false);
...
bool blk_zone_write_plug_bio(struct bio *bio)
{
	 ...
	 * plug bio submit work. Since we already acconted for it, fix the
	 * queue usage counter and let it execute.
	 */
	...
	/*
	 * Get the zone write plug for the BIO. The zone write plug will be
	 * locked and activated.
	 */
	zwplug = blk_zone_lookup_wplug(bio, true);
	if (!zwplug)
		return false;
...
void blk_zone_write_plug_split_bio(struct bio *bio, struct bio *split)
{
	 ...
	 * original bio must thus be plugged again and the first part split off
	 * it be allowed to proceed by accounting for it and marking split has
	 * having already been handled by the zone write plug.
	 */
	blk_zone_wplug_lock(zwplug);
	...
	percpu_ref_get(&split->bi_bdev->bd_disk->queue->q_usage_counter);
