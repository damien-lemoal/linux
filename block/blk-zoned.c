...
static inline void blk_zone_wplug_set_flag(struct blk_zone_wplug *zwplug,
					   unsigned int bit)
{
	zwplug->flags |= (1U << bit);
}

static inline void blk_zone_wplug_clear_flag(struct blk_zone_wplug *zwplug,
					     unsigned int bit)
{
	zwplug->flags &= ~(1U << bit);
}

static inline bool blk_zone_wplug_flagged(struct blk_zone_wplug *zwplug,
					  unsigned int bit)
{
	return zwplug->flags & (1U << bit);
}
				    ...
				    struct blk_zone_wplug *zwplug)
{
	...
	if (blk_zone_wplug_flagged(zwplug, BLK_ZONE_WPLUG_ACTIVE))
		return true;
	...
	blk_zone_wplug_set_flag(zwplug, BLK_ZONE_WPLUG_ACTIVE);

	return true;
}

static int blk_zone_free_active_wplug(struct gendisk *disk,
				      struct blk_zone_active_wplug *zawplug)
{
	struct blk_zone_wplug *zwplug = zawplug->zwplug;
	int n;

	n = blk_zone_abort_active_wplug(disk, zawplug, true);

	blk_zone_wplug_clear_flag(zwplug, BLK_ZONE_WPLUG_PLUGGED);
	blk_zone_wplug_clear_flag(zwplug, BLK_ZONE_WPLUG_ACTIVE);
...
__blk_zone_lookup_wplug(struct gendisk *disk, sector_t sector)
{
	struct blk_zone_wplug *zwplug =
		&disk->zone_wplugs[disk_zone_no(disk, sector)];

	if (blk_zone_wplug_flagged(zwplug, BLK_ZONE_WPLUG_CONV))
		return NULL;
	return zwplug;
}
...
{
	return __blk_zone_lookup_wplug(bio->bi_bdev->bd_disk,
...
bool blk_zone_wplug_plugged(struct gendisk *disk, unsigned int zno)
{
	...
	return blk_zone_wplug_flagged(zwplug, BLK_ZONE_WPLUG_PLUGGED);
}
			   ...
			   unsigned int *wp_offset)
{
	...
	active = blk_zone_wplug_flagged(zwplug, BLK_ZONE_WPLUG_ACTIVE);
	if (active)
					 ...
					 bool abort_all)
{
	 ...
	 */
	if (blk_zone_wplug_flagged(zwplug, BLK_ZONE_WPLUG_ERROR)) {
		blk_zone_wplug_clear_flag(zwplug, BLK_ZONE_WPLUG_ERROR);
	...
	/*
	 * Active zone: update the write pointer and based on the value,
	 * mark the zone as full and/or abort plugged regular writes
	 * that are unaligned.
	 */
	...
	/*
	 * If the zone is full or empty with no remaining BIOs, free
	 * its active write plug.
	 */
	if (zawplug->wp_offset >= zawplug->capacity
	    || (!wp_offset && bio_list_empty(&zawplug->bio_list)))
...
void blk_zone_write_plug_bio_split(struct bio *bio, struct bio *split)
{
	struct blk_zone_wplug *zwplug = blk_zone_lookup_wplug(split);
	...
	/*
	 * A plugged original bio that was unplugged and submitted was split
	 * into split + smaller bio. split must be executed so we mark it as
	 * having gone through the plug already. The remainder bio is plugged
	 * again. and will execute after split completes.
	 */
	blk_zone_wplug_add_bio(zwplug, bio);
	bio_set_flag(split, BIO_ZONE_WRITE_PLUGGING);

	/*
	 * When the original bio was submitted, blk_zone_wplug_prepare_bio()
	 * advanced the zone write plug write pointer offset using the original
	 * BIO size, but it is now smaller. Correct this.
	 */
	if (WARN_ON_ONCE(zawplug->wp_offset < bio_sectors(bio))) {
		/* This should not happen, so trigger errors and recovery. */
...
{
	struct blk_zone_wplug *zwplug = blk_zone_lookup_wplug(req->bio);
	struct blk_zone_active_wplug *zawplug = zwplug->zawplug;
	...
	while (zawplug->wp_offset < zawplug->capacity &&
		 ...
		 * unplug path to signal the user the write sequence is invalid.
		 */
		...
		if (bio_rw_needs_split(bio, &nr_segs))
			break;

		bio_list_pop(&zawplug->bio_list);
		ret = blk_attempt_bio_merge(q, req, bio, nr_segs, false);
		if (ret != BIO_MERGE_OK) {
...
{
	if (!blk_zone_wplug_flagged(zwplug, BLK_ZONE_WPLUG_ERROR)) {
		blk_zone_wplug_set_flag(zwplug, BLK_ZONE_WPLUG_ERROR);
...
static bool blk_zone_wplug_handle_write(struct bio *bio)
{
	struct blk_zone_wplug *zwplug;
	unsigned long flags;
	 ...
	 */
	if (blk_zone_wplug_flagged(zwplug, BLK_ZONE_WPLUG_PLUGGED) ||
	    blk_zone_wplug_flagged(zwplug, BLK_ZONE_WPLUG_ERROR)) {
		blk_zone_wplug_set_flag(zwplug, BLK_ZONE_WPLUG_PLUGGED);
		blk_zone_wplug_add_bio(zwplug, bio);
		blk_zone_wplug_unlock(zwplug, flags);
		return true;
	}
...
bool blk_zone_write_plug_bio(struct bio *bio)
{
	...
	case REQ_OP_WRITE_ZEROES:
		return blk_zone_wplug_handle_write(bio);
...
void blk_zone_write_plug_bio_endio(struct bio *bio)
{
	...
	/* If bio failed, mark the plug as having an error. */
...
void blk_zone_write_plug_complete_request(struct request *req)
{
	...
	struct blk_zone_wplug *zwplug =
		__blk_zone_lookup_wplug(disk, req->__sector);
	struct blk_zone_active_wplug *zawplug = zwplug->zawplug;
	unsigned long flags;
				      ...
				      unsigned int nr_zones)
{
	...
	for (i = 0; i < nr_zones; i++, zwplug++) {
		if (!blk_zone_wplug_flagged(zwplug, BLK_ZONE_WPLUG_ACTIVE))
			continue;
		blk_zone_wplug_lock(zwplug, flags);
		n = blk_zone_free_active_wplug(disk, zwplug->zawplug);
		blk_zone_wplug_unlock(zwplug, flags);
		if (n)
				  ...
				  void *data)
{
	...
	case BLK_ZONE_TYPE_CONVENTIONAL:
		blk_zone_wplug_set_flag(&args->zone_wplugs[idx],
					BLK_ZONE_WPLUG_CONV);
		break;
