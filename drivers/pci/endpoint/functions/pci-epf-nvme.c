					     ...
					     size_t transfer_len)
{
	...
	if (transfer_len > prp_size) {
		/*
		 * We should have a valid prp2, unles this is an admin command
		 * which ddoes not allow crossing page boundaries.
		 */
		if (!epcmd->qid)
			goto invalid_field;
				    ...
				    phys_addr_t pci_addr)
{
	...
	ret = pci_epf_nvme_map(nvme, &cq->map, pci_addr, qsize);
	if (ret) {
				    ...
				    phys_addr_t pci_addr)
{
	...
	ret = pci_epf_nvme_map(nvme, &sq->map, pci_addr, qsize);
	if (ret) {
