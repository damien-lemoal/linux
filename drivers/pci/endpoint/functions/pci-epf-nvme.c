...
static int pci_epf_nvme_create_ctrl(struct pci_epf *epf)
{
	...
	/*
	 * If we already have our fabrics controller, we only
	 * need to re-initialize the PCI controller registers.
	 */
...
static int pci_epf_nvme_alloc_reg_bar(struct pci_epf *epf)
{
	...
	if (!epf_nvme->max_nr_queues) {
		dev_err(&epf->dev, "Maximum number of queues is 0\n");
		return -EINVAL;
	}
