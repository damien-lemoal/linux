...
/*
 * Map @size bytes from @pci_addr in the host PCI address space o CPU memory.
			    ...
			    struct pci_epc_map *map)
{
	...
	/* Allocate and map local memory */
	ret = pci_epf_mem_map(epf, pci_addr, size, map);
	if (ret) {
		dev_err(&epf->dev,
			"Allocate and map host PCI address failed\n");
		return ret;
	}

	dev_dbg(&nvme->epf->dev,
		"Map PCI addr 0x%llx + %zu B -> phys 0x%llx, virt 0x%lx, ofst 0x%llx %zu B\n",
		map->pci_addr, size, map->phys_base,
		(unsigned long)map->virt_base,
		map->phys_addr - map->phys_base, map->phys_size);
...
static void pci_epf_nvme_unmap(struct pci_epf_nvme *nvme,
			       struct pci_epc_map *map)
{
	pci_epf_mem_unmap(nvme->epf, map);
				 ...
				 enum dma_data_direction dir, void *buf)
{
	...
	/* Map segment */
	ret = pci_epf_nvme_map(nvme, seg->pci_addr, seg->size, &map);
	if (ret)
		return ret;
				     ...
				     size_t mapping_size)
{
	...
	prp_size = min(pci_epf_nvme_prp_size(prp),
		       (mapping_size >> NVME_CTRL_PAGE_SHIFT) << 3);
	ret = pci_epf_nvme_map(nvme, prp, prp_size, &map);
	if (ret)
		return ret;
				    ...
				    phys_addr_t pci_addr)
{
	...
	ret = pci_epf_nvme_map(nvme, pci_addr, qsize, &cq->map);
	if (ret) {
				    ...
				    phys_addr_t pci_addr)
{
	...
	ret = pci_epf_nvme_map(nvme, pci_addr, qsize, &sq->map);
	if (ret) {
