				     ...
				     void *buf, enum dma_data_direction dir)
{
	...
	dma_phys_addr = dma_map_single(nvme->dma_dev, buf, map->size, dir);
	...
	dma_unmap_single(nvme->dma_dev, dma_phys_addr, map->size, dir);
				 ...
				 enum dma_data_direction dir, void *buf)
{
	...
	while (size) {
		...
		if (!nvme->dma_enable || map.size < ctrl->mps)
			ret = pci_epf_nvme_mmio_transfer(nvme, &map, buf, dir);
		else
			ret = pci_epf_nvme_dma_transfer(nvme, &map, buf, dir);
...
static void pci_epf_nvmet_cleanup(struct pci_epf_nvme *nvme)
{
	...
	if (target->subsys) {
		list_del_init(&host_link->entry);
		list_del_init(&subsys_link->entry);
