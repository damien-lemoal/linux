...
void mmc_command_done(struct mmc_host *host, struct mmc_request *mrq)
{
	...
	pr_info("%s: cmd done, tfr ongoing (CMD%u)\n",
...
static void mmc_mrq_pr_info(struct mmc_host *host, struct mmc_request *mrq,
			     bool cqe)
{
	if (mrq->sbc) {
	...
	if (mrq->cmd) {
		pr_info("%s: starting %sCMD%u arg %08x flags %08x\n",
			 mmc_hostname(host), cqe ? "CQE direct " : "",
			 mrq->cmd->opcode, mrq->cmd->arg, mrq->cmd->flags);
	} else if (cqe) {
		pr_info("%s: starting CQE transfer for tag %d blkaddr %u\n",
			 mmc_hostname(host), mrq->tag, mrq->data->blk_addr);
	}

	if (mrq->data) {
		pr_info("%s:     blksz %d blocks %d flags %08x "
			"tsac %d ms nsac %d\n",
	...
	if (mrq->stop) {
		pr_info("%s:     CMD%u arg %08x flags %08x\n",
			 mmc_hostname(host), mrq->stop->opcode,
...
int mmc_start_request(struct mmc_host *host, struct mmc_request *mrq)
{
	...
	mmc_mrq_pr_info(host, mrq, false);
...
void mmc_cqe_request_done(struct mmc_host *host, struct mmc_request *mrq)
{
	...
	if (mrq->cmd) {
		pr_info("%s: CQE req done (direct CMD%u): %d\n",
			 mmc_hostname(host), mrq->cmd->opcode, mrq->cmd->error);
	} else {
		pr_info("%s: CQE transfer done tag %d\n",
