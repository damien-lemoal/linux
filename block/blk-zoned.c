 ...
 *  - BLK_ZONE_WPLUG_RECLAIM: Indicate that an active zone can have its
 *    struct blk_zone_active_wplug reclaimed (freed) because the zone is empty
 *    or full.
 */
enum {
...
static void blk_zone_abort_wplug(struct blk_zone_wplug *zwplug)
{
	...
	while ((bio = bio_list_pop(&zwplug->zawplug->bio_list))) {
		...
		percpu_ref_put(&disk->queue->q_usage_counter);
...
/*
 * Unit for growing/shrinking a disk pool of active write plugs.
 */
#define BLK_ZONE_WPLUG_POOL_RESIZE_NR 	(128)

/*
 * Check if we should run active zone write plug work to
 * reclaim active plugs and resize the pool.
 */
static void blk_zone_run_active_wplugs_reclaim(struct gendisk *disk)
{
	unsigned int reclaim_nr = atomic_read(&disk->zone_awplugs_reclaim_nr);
	unsigned int used_nr = atomic_read(&disk->zone_awplugs_nr);
	unsigned long delay = HZ;

	/*
	 * If we have nothing to reclaim, then do nothing if the pool is not
	 * fully used. Otherwise, we need to grow the pool.
	 */
	if (!reclaim_nr) {
		if (used_nr <= disk->zone_awplugs_pool_size)
			return;
		delay = 0;
	} else {
		/* Do not shrink the pool too aggressively. */
		if (used_nr && reclaim_nr < BLK_ZONE_WPLUG_POOL_RESIZE_NR)
			return;
	}

	if (!delay || !work_busy(&disk->zone_awplugs_work.work))
		kblockd_mod_delayed_work_on(WORK_CPU_UNBOUND,
					    &disk->zone_awplugs_work, delay);
}
 ...
 */
static void blk_zone_activate_wplug(struct gendisk *disk,
				    struct blk_zone_wplug *zwplug)
{
	struct blk_zone_active_wplug *zawplug;
	gfp_t gfp_mask = GFP_NOWAIT;

	/* If we have an active write plug already, keep using it. */
	if (test_bit(BLK_ZONE_WPLUG_ACTIVE, &zwplug->flags)) {
		if (test_and_clear_bit(BLK_ZONE_WPLUG_RECLAIM, &zwplug->flags))
			atomic_dec(&disk->zone_awplugs_reclaim_nr);
		return;
	}

	blk_zone_run_active_wplugs_reclaim(disk);
	...
	while (1) {
		/*
		 * Another context may have activated the zone while we had
		 * the zone write plug lock released.
		 */
		if (test_bit(BLK_ZONE_WPLUG_ACTIVE, &zwplug->flags))
			return;

		zawplug = mempool_alloc(disk->zone_awplugs_pool, gfp_mask);
		if (zawplug)
			break;

		blk_zone_wplug_unlock(zwplug);

		/* Schedule immediate reclaim */
		if (!work_busy(&disk->zone_awplugs_work.work))
			kblockd_mod_delayed_work_on(WORK_CPU_UNBOUND,
						&disk->zone_awplugs_work, 0);

		io_schedule();

		flush_delayed_work(&disk->zone_awplugs_work);
	...
	atomic_inc(&disk->zone_awplugs_nr);
...
/*
 * Free the active write plug of zone write plugs that are marked with
 * the BLK_ZONE_WPLUG_DEACTIVATE flag.
 */
static void blk_zone_reclaim_active_wplug(struct gendisk *disk,
					  struct blk_zone_wplug *zwplug)
{
	struct blk_zone_active_wplug *zawplug = zwplug->zawplug;

	/*
	 * Even if the plug is marked for reclaim, it may still be plugged.
	 * Given that the plugged flag is cleared in the bio work, also
	 * check that this work is done.
	 */
	if (test_bit(BLK_ZONE_WPLUG_PLUGGED, &zwplug->flags) ||
	    work_busy(&zawplug->bio_work))
		return;

	/* The active zone write plug may have been reused. */
	if (!test_and_clear_bit(BLK_ZONE_WPLUG_RECLAIM, &zwplug->flags))
		return;
	...
	atomic_dec(&disk->zone_awplugs_nr);
}

/*
 * Check if a zone write plug is still active, that is, if its write pointer
 * offset is not 0 and less than the zone capacity. If that is not the case,
 * mark the active zone for reclaiming and schedule the disk write plug
 * reclaim work.
 *
 * The zone write plug must be locked.
 */
static void blk_zone_wplug_check_reclaim(struct gendisk *disk,
					 struct blk_zone_wplug *zwplug)
{
	struct blk_zone_active_wplug *zawplug;

	if (!test_bit(BLK_ZONE_WPLUG_ACTIVE, &zwplug->flags) ||
	    test_bit(BLK_ZONE_WPLUG_RECLAIM, &zwplug->flags))
		return;
	...
	/* Zone empty or full: reclaim its active zone wplug. */
	set_bit(BLK_ZONE_WPLUG_RECLAIM, &zwplug->flags);
	atomic_inc(&disk->zone_awplugs_reclaim_nr);

	blk_zone_run_active_wplugs_reclaim(disk);
}

static void blk_zone_active_wplugs_resize_pool(struct gendisk *disk,
					       unsigned int new_size)
{
	unsigned int pool_size = disk->zone_awplugs_pool_size;
	unsigned int zawp_nr = atomic_read(&disk->zone_awplugs_nr);
	unsigned int min_pool_size;
	int ret;

	if (new_size) {
		/* Disk initialization: set initial pool size. */
		pool_size = new_size;
	} else {
		/* Runtime grow/shring case. */
		if (zawp_nr >= pool_size)
			pool_size += BLK_ZONE_WPLUG_POOL_RESIZE_NR;
		else if (zawp_nr + BLK_ZONE_WPLUG_POOL_RESIZE_NR <= pool_size)
			pool_size = zawp_nr;
	}

	min_pool_size = max(disk->max_active_zones,
			    disk->max_open_zones);
	if (!min_pool_size)
		min_pool_size = BLK_ZONE_WPLUG_POOL_RESIZE_NR;
	pool_size = max(pool_size, min_pool_size);
	pool_size = min(pool_size, disk->nr_zones);

	if (pool_size == disk->zone_awplugs_pool_size)
		return;
...
static void blk_zone_active_wplugs_work(struct work_struct *work)
{
	struct gendisk *disk =
		container_of(work, struct gendisk, zone_awplugs_work.work);
	struct blk_zone_wplug *zwplug;
	unsigned int zno = 0;

	/* Serialize against revalidate swapping the write plugs array. */
	...
	while (zno < disk->nr_zones &&
	       atomic_read(&disk->zone_awplugs_reclaim_nr)) {
		if (test_bit(BLK_ZONE_WPLUG_RECLAIM, &zwplug->flags) &&
		    !test_bit(BLK_ZONE_WPLUG_LOCKED, &zwplug->flags) &&
		    !test_bit(BLK_ZONE_WPLUG_PLUGGED, &zwplug->flags)) {
			blk_zone_wplug_lock(zwplug);
			blk_zone_reclaim_active_wplug(disk, zwplug);
			blk_zone_wplug_unlock(zwplug);
		}
		zno++;
		zwplug++;
	...
	blk_zone_active_wplugs_resize_pool(disk, 0);
					 ...
					 unsigned int wp_offset)
{
	...
	if (test_bit(BLK_ZONE_WPLUG_ACTIVE, &zwplug->flags)) {
		zwplug->zawplug->wp_offset = wp_offset;
		blk_zone_wplug_check_reclaim(disk, zwplug);
	} else {
					  ...
					  unsigned int idx, void *data)
{
	...
	/* Update the zone write plug capacity and write pointer offset. */
	if (test_bit(BLK_ZONE_WPLUG_ACTIVE, &zwplug->flags)) {
...
static bool blk_zone_handle_write_error(struct gendisk *disk,
					struct blk_zone_wplug *zwplug)
{
					  ...
					  struct bio *bio)
{
	...
	/*
	 * Keep a reference on the request queue of the BIO until the BIO is
	 * remove from the zone write plug BIO list.
	 */
...
static inline bool blk_zone_wplug_has_error(struct gendisk *disk,
					    struct blk_zone_wplug *zwplug)
{
	struct blk_zone_active_wplug *zawplug = zwplug->zawplug;
	...
	if (test_bit(BLK_ZONE_WPLUG_ERROR, &zwplug->flags) &&
	    !blk_zone_handle_write_error(disk, zwplug))
		return true;
	 ...
	 * directed at the following zone.
         */
	return zawplug->wp_offset >= zawplug->capacity;
}
 ...
 */
static void blk_zone_wplug_prepare_bio(struct blk_zone_wplug *zwplug,
				       struct bio *bio)
{
...
bool blk_zone_write_plug_bio(struct bio *bio)
{
	...
	if (!disk->zone_wplugs || !bio->bi_bdev)
		return false;
	 ...
	 * and may straddling the zone boundary. It is the responsibility of
	 * the driver to split BIOs before plugging them. Fail any BIO that is
	 * not fully contained in a single zone.
	 */
	if (bdev->bd_has_submit_bio && bio_straddle_zones(bio)) {
		bio_io_error(bio);
		return true;
	}
	...
	blk_zone_wplug_lock(zwplug);
	blk_zone_activate_wplug(disk, zwplug);
	 ...
	 * Otherwise, add it to the plug bio list.
	 */
	...
	if (blk_zone_wplug_has_error(disk, zwplug)) {
	...
	blk_zone_wplug_prepare_bio(zwplug, bio);
...
void blk_zone_write_plug_split_bio(struct bio *bio, struct bio *split)
{
	struct blk_zone_wplug *zwplug = blk_zone_lookup_wplug(bio);
	struct blk_zone_active_wplug *zawplug = zwplug->zawplug;
	...
	/*
	 * While splitting is first done in blk_mq_submit_bio()
	 * when a BIO is first submitted, splitting may again happen when
	 * the zone plug submit work unplugs and submits a BIO, if the disk
	 * queue limits change while the BIO was plugged. The now smaller
	 * original bio must thus be plugged again. The first part that was
	 * split off it is allowed to execute by marking split has having
	 * already been handled by the zone write plug.
	 */
	blk_zone_wplug_lock(zwplug);

	blk_zone_wplug_add_bio(zwplug, bio);

	bio_set_flag(split, BIO_ZONE_WRITE_PLUGGING);

	/*
	 * We advanced the zone write pointer offset by the size of original
	 * BIO when it was unplugged. However, we are now going to issue only
	 * the "split" part of it. Correct the zone write pointer offset
	 * accordingly.
	 */
	if (WARN_ON_ONCE(zawplug->wp_offset <= nr_sectors)) {
		set_bit(BLK_ZONE_WPLUG_ERROR, &zwplug->flags);
		zawplug->wp_offset = 0;
	} else {
		zawplug->wp_offset -= nr_sectors;
	}
...
void blk_zone_write_bio_endio(struct bio *bio)
{
	...
	/*
	 * If bio failed, mark the zone write plug as having an error so that
	 * when the next bio is issued, the bio work updates the write pointer
	 * offset using report zones.
	 */
				       ...
				       unsigned int nr_zones)
{
	...
	for (i = 0; i < nr_zones; i++, zwp++) {
		...
		if (!test_and_set_bit(BLK_ZONE_WPLUG_RECLAIM, &zwp->flags))
			atomic_inc(&disk->zone_awplugs_reclaim_nr);
		clear_bit(BLK_ZONE_WPLUG_PLUGGED, &zwp->flags);
		blk_zone_reclaim_active_wplug(disk, zwp);
...
static int disk_init_zone_resources(struct gendisk *disk, unsigned int nr_zones)
{
	...
	/*
	  * Create the disk pool of active zone write plugs with enough elements
	...
	INIT_DELAYED_WORK(&disk->zone_awplugs_work, blk_zone_active_wplugs_work);
	atomic_set(&disk->zone_awplugs_nr, 0);
	atomic_set(&disk->zone_awplugs_reclaim_nr, 0);
...
{
	if (disk->zone_awplugs_pool)
		cancel_delayed_work_sync(&disk->zone_awplugs_work);
...
struct blk_revalidate_zone_args {
	...
	unsigned int	nr_active_zwplugs;
	sector_t	sector;
};
				  ...
				  void *data)
{
	...
	case BLK_ZONE_TYPE_SEQWRITE_REQ:
		...
		args->zone_wplugs[idx].info.wp_offset = wp_offset;
		if (wp_offset && wp_offset < zone->capacity)
			args->nr_active_zwplugs++;
		break;
...
int blk_revalidate_disk_zones(struct gendisk *disk)
{
	 ...
	 */
	ret = disk_init_zone_resources(disk, nr_zones);
	if (ret)
		return ret;
	...
	if (ret > 0) {
		...
		/*
		 * Check again the size of the pool of active zone write plugs using
		 * the number of active zones we counted. Generously resize the pool
		 * if we found more active zones than first estimated.
		 */
		if (args.nr_active_zwplugs > disk->zone_awplugs_pool_size)
			blk_zone_active_wplugs_resize_pool(disk,
						   args.nr_active_zwplugs * 2);
		ret = 0;
	} else {
