...
{
	struct pci_epf_nvme_queue *cq =
		container_of(work, struct pci_epf_nvme_queue, work);
	struct pci_epf_nvme_cmd *epcmd;
	unsigned long flags;
	LIST_HEAD(list);
...
pci_epf_nvme_alloc_queues(struct pci_epf_nvme *epf_nvme, int nr_queues)
{
	...
	for (i = 0; i < nr_queues; i++) {
		...
		INIT_WORK(&q[i].work, pci_epf_nvme_cq_work);
				      ...
				      bool shutdown)
{
	...
	if (shutdown) {
		/* Delete the controller to cancel all async event commands */
		nvme_delete_ctrl_sync(ctrl->ctrl);
		ctrl->ctrl = NULL;
	}
...
out_reg_wq:
	...
	pr_info("Register nvme EPF driver failed\n");
