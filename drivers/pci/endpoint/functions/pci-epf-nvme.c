...
#define PCI_EPF_NVME_MAX_PRPS		\
	((PCI_EPF_NVME_MDTS / NVME_CTRL_PAGE_SIZE) + 1)

/* Maximum number of prps in a page per nvme */
#define PCI_EPF_NVME_PRP_PER_PAGE	(NVME_CTRL_PAGE_SIZE / sizeof(__le64)))
...
/*
 * Map @size bytes from @pci_addr in the host PCI space.
 * The local mapped memory is indicated by map->virt_addr.
 */
			    ...
			    bool zero)
{
	...
	align = max_t(size_t, nvme->epc_features->align, NVME_CTRL_PAGE_SIZE);
	map->pci_addr = pci_addr;
	map->pci_base = ALIGN_DOWN(pci_addr, align);
	map->pci_ofst = map->pci_addr - map->pci_base;
	map->size = ALIGN(map->pci_ofst + size, align);

	map->virt_base = pci_epc_mem_alloc_addr(epc, &map->phys_base,
						map->size);
	if (!map->virt_base) {
		dev_err(&epf->dev, "Allocate host PCI map address failed\n");
		return -ENOMEM;
	}

	ret = pci_epc_map_addr(epf->epc, epf->func_no, epf->vfunc_no,
			       map->phys_base, map->pci_base, map->size);
	if (ret) {
		dev_err(&epf->dev, "Map host phys address failed\n");
		pci_epc_mem_free_addr(epc, map->phys_base, map->virt_base,
				      map->size);
		return ret;
	}

	map->phys_addr = map->phys_base + map->pci_ofst;
	map->virt_addr = map->virt_base + map->pci_ofst;

	if (zero)
		memset_io(map->virt_addr, 0, size);

	dev_dbg(&epf->dev,
		"Map PCI 0x%llx + %zu B, align %zu -> base 0x%llx, ofst %zu, "
		"size %zu B, virt addr 0x%llx\n",
		pci_addr, size, align, map->pci_base, map->pci_ofst, map->size,
		(u64)map->virt_addr);
			       ...
			       struct pci_epf_nvme_map *map)
{
	...
	pci_epc_unmap_addr(epc, epf->func_no, epf->vfunc_no, map->phys_base);
	pci_epc_mem_free_addr(epc, map->phys_base, map->virt_base,
			      map->size);
}
...
static bool pci_epf_nvme_init_dma(struct pci_epf_nvme *nvme)
{
	...
	dma_chan = dma_request_channel(mask, pci_epf_nvme_dma_filter_fn,
				       &filter);
	if (!dma_chan) {
		dev_info(dev, "Using generic DMA channel as rx channel\n");
		goto fail_back_tx;
	}
	...
	dma_chan = dma_request_channel(mask, pci_epf_nvme_dma_filter_fn,
				       &filter);
	if (!dma_chan) {
		dev_info(dev, "Using generic DMA channel as tx channel\n");
		goto fail_back_rx;
	}
...
static void pci_epf_nvme_dma_callback(void *param)
{
        ...
        nvme->dma_status =
		dmaengine_tx_status(nvme->dma_chan, nvme->dma_cookie, &state);
	if (nvme->dma_status == DMA_COMPLETE || nvme->dma_status == DMA_ERROR)
			    ...
			    enum dma_transfer_direction dir)
{
	...
	wait_for_completion(&nvme->dma_complete);

	if (nvme->dma_status == DMA_ERROR) {
		dev_err(&nvme->epf->dev, "DMA transfer failed\n");
		ret = -EIO;
	}

	WARN_ON(nvme->dma_status != DMA_COMPLETE);
...
{
	struct device *dma_dev = nvme->epf->epc->dev.parent;
	phys_addr_t dma_phys_addr;
	int ret;

	dma_phys_addr = dma_map_single(dma_dev, buf, size, dir);
	if (dma_mapping_error(dma_dev, dma_phys_addr)) {
	...
	dma_unmap_single(dma_dev, dma_phys_addr, size, dir);
				 ...
				 void *buf)
{
	...
	/* Map segment */
	ret = pci_epf_nvme_map(nvme, &map, seg->pci_addr, seg->size, false);
	if (ret)
		return ret;
				     ...
				     enum dma_data_direction dir)
{
	...
	for (i = 0; i < epcmd->nr_segs; i++) {
		...
		dev_dbg(&nvme->epf->dev,
			"Transfer %zu B -> %zu B / %zu B\n",
			seg->size, size, epcmd->buffer_size);
	}
...
static void pci_epf_nvme_cmd_complete(struct pci_epf_nvme_cmd *epcmd)
{
	...
	dev_dbg(&epf->dev, "cq[%d]: status 0x%x, phase %d, tail %d -> %d/%d\n",
		epcmd->qid, epcmd->status, cq->phase, cq->tail,
		(int)cq->tail, (int)cq->depth);

	memcpy_toio(cq->map.virt_addr + cq->tail * cq->qes, cqe, cq->qes);
...
static int pci_epf_nvme_get_prp_list(struct pci_epf_nvme *nvme,
				     struct pci_epf_nvme_cmd *epcmd, u64 prp,
				     __le64 **prp_list, int *nr_prps)
{
	struct pci_epf_nvme_seg seg;
	__le64 *prps;
	int ret;

	/* Allocate a buffer to get the prp list */
	prps = kzalloc(NVME_CTRL_PAGE_SIZE, GFP_KERNEL);
	if (!prps) {
	...
	/* Get the list */
	seg.pci_addr = prp;
	seg.size = NVME_CTRL_PAGE_SIZE - pci_epf_nvme_prp_ofst(prp);

	ret = pci_epf_nvme_transfer(nvme, &seg, DMA_FROM_DEVICE, prps);
	if (ret) {
		dev_err(&nvme->epf->dev, "Transfer prp list failed\n");
		epcmd->status = NVME_SC_DATA_XFER_ERROR | NVME_SC_DNR;
		kfree(prps);
		return ret;
	}

	*nr_prps = seg.size / sizeof(__le64);
	*prp_list = prps;

	return 0;
}

static int pci_epf_nvme_cmd_setup_prp_list(struct pci_epf_nvme *nvme,
					   struct pci_epf_nvme_cmd *epcmd,
					   size_t transfer_len)
{
	struct nvme_command *cmd = &epcmd->cmd;
	struct pci_epf_nvme_seg *seg;
	size_t ofst, size = 0, prp_size;
	phys_addr_t pci_addr;
	int nr_segs, nr_prps;
	__le64 *prps = NULL;
	int i = 0, ret;
	...
	seg = &epcmd->segs[0];
	seg->pci_addr = prp;
	seg->size = NVME_CTRL_PAGE_SIZE - ofst;
	size = seg->size;
	pci_addr = seg->pci_addr + seg->size;
	nr_segs = 1;
	...
	while (size < transfer_len) {
		...
		if (!prps) {
			/* Get next prp list */
			ret = pci_epf_nvme_get_prp_list(nvme, epcmd, prp,
						&prps, &nr_prps);
			if (ret)
				return ret;
			i = 0;
		...
		if (i == nr_prps - 1 ||
		    (i < nr_prps - 1 && !le64_to_cpu(prps[i + 1]))) {
			if (transfer_len - size > NVME_CTRL_PAGE_SIZE) {
				/* prp is a list pointer */
				kfree(prps);
				prps = NULL;
				continue;
			}
		...
		if (prp == pci_addr) {
			/* Contiguous prp: add it to the current segment */
			seg->size += prp_size;
			pci_addr += prp_size;
		} else {
			/* Discontiguous prp: new segment */
			nr_segs++;
			if (nr_segs > epcmd->nr_segs) {
				dev_err(&nvme->epf->dev,
					"Too many segments %d/%d\n",
					nr_segs, epcmd->nr_segs);
				goto internal;
			}
...
static int pci_epf_nvme_cmd_setup_prp_simple(struct pci_epf_nvme *nvme,
					     struct pci_epf_nvme_cmd *epcmd,
					     size_t transfer_len)
{
	struct nvme_command *cmd = &epcmd->cmd;
	u64 prp1, prp2 = 0, ofst;
	int ret, nr_segs = 1;

	/* prp1 */
	prp1 = le64_to_cpu(cmd->common.dptr.prp1);
	ofst = pci_epf_nvme_prp_ofst(prp1);
	if (transfer_len > NVME_CTRL_PAGE_SIZE - ofst) {
		/* We should have a valid prp2 */
		prp2 = le64_to_cpu(cmd->common.dptr.prp2);
		if (!prp2)
			goto invalid_field;
		if (pci_epf_nvme_prp_ofst(prp2))
			goto invalid_offset;
		if (prp2 != pci_epf_nvme_prp_addr(prp1) + NVME_CTRL_PAGE_SIZE)
			nr_segs = 2;
	}
	...
	if (nr_segs == 1) {
		...
		epcmd->segs[0].size = NVME_CTRL_PAGE_SIZE - ofst;
		epcmd->segs[1].pci_addr = prp2;
		epcmd->segs[1].size = transfer_len - epcmd->segs[0].size;
	}
...
static int pci_epf_nvme_cmd_setup(struct pci_epf_nvme *nvme,
				  struct pci_epf_nvme_cmd *epcmd,
				  size_t transfer_len)
{
	struct nvme_command *cmd = &epcmd->cmd;
	...
	if (transfer_len + ofst <= (NVME_CTRL_PAGE_SIZE * 2))
		ret = pci_epf_nvme_cmd_setup_prp_simple(nvme, epcmd,
						        transfer_len);
	else
		ret = pci_epf_nvme_cmd_setup_prp_list(nvme, epcmd,
						      transfer_len);
	if (ret)
		return ret;
...
static void pci_epf_nvmet_queue_response(struct nvmet_req *req)
{
	...
	if (epcmd->status)
		dev_err(&epcmd->nvme->epf->dev,
			"QID %d: cmd 0x%x failed, status 0x%0x\n",
			epcmd->qid, epcmd->cmd.common.opcode,
			epcmd->status);
...
static int pci_epf_nvme_target_connect(struct pci_epf_nvme *nvme)
{
	...
	dev_info(&epf->dev,
		 "Connected to target controller %d\n",
		 target->nvmet_ctrl->cntlid);
...
static void pci_epf_nvme_target_disconnect(struct pci_epf_nvme *nvme)
{
	...
	dev_info(&nvme->epf->dev,
		 "Disconnect target controller %d\n",
		 target->nvmet_ctrl->cntlid);
					      ...
					      int qid)
{
	...
	ret = pci_epf_nvme_map(nvme, &sq->map, sq->pci_addr, qsize, true);
	if (ret) {
	...
	ret = pci_epf_nvme_map(nvme, &cq->map, cq->pci_addr, qsize, true);
	if (ret) {
						 ...
						 int qid)
{
	...
	pci_epf_nvme_unmap(nvme, &cq->map);
...
static void pci_epf_nvme_ctrl_enable(struct pci_epf_nvme *nvme)
{
	...
	if (ret) {
		dev_err(&epf->dev, "Connect to target failed\n");
		return;
	}

	ctrl->mps = 1LU << (nvmet_cc_mps(ctrl->cc) + 12);

	dev_dbg(&epf->dev, "MPS: %zu B\n", ctrl->mps);
	...
	/*
	 * Hack: Let things settle down on the link. otherwise command
	 * fetching from the admin SQ generates an NMI sometimes...
	 */
	msleep(50);
...
static void pci_epf_nvme_exec_cmd(struct pci_epf_nvme *nvme,
				  struct pci_epf_nvme_cmd *epcmd, bool async)
{
	struct pci_epf_nvme_target *target = &nvme->target;
	struct nvmet_req *req = &epcmd->req;
	struct nvme_command *cmd = req->cmd;
...
static void pci_epf_nvme_target_keep_alive(struct pci_epf_nvme *nvme)
{
	...
	pci_epf_nvme_exec_cmd(nvme, epcmd, false);
					 ...
					 struct pci_epf_nvme_cmd *epcmd)
{
	...
	cq->flags = cq_flags;
	cq->vector = vector;
	cq->pci_addr = le64_to_cpu(cmd->create_cq.prp1);
}
					 ...
					 struct pci_epf_nvme_cmd *epcmd)
{
	...
	sq->size = qsize;
	sq->flags = sq_flags;
	sq->pci_addr = le64_to_cpu(cmd->create_sq.prp1);
...
static struct pci_epf_nvme_cmd *
pci_epf_nvme_fetch_command(struct pci_epf_nvme *nvme, int qid)
{
	struct pci_epf_nvme_ctrl *ctrl = &nvme->ctrl;
	struct pci_epf_nvme_queue *sq = &ctrl->sq[qid];
	struct pci_epf_nvme_cmd *epcmd;
					     ...
					     struct pci_epf_nvme_cmd *epcmd)
{
	...
	/* Limit maximum command size to PCI_EPF_NVME_MDTS */
	id->mdts = ilog2(PCI_EPF_NVME_MDTS >> NVME_CTRL_PAGE_SHIFT);
}
...
static bool pci_epf_nvme_process_admin_cmd(struct pci_epf_nvme *nvme)
{
	...
	epcmd = pci_epf_nvme_fetch_command(nvme, 0);
	if (!epcmd)
		return false;
	...
	if (cmd->common.flags & NVME_CMD_SGL_ALL) {
		dev_err(&nvme->epf->dev, "Invalid non-PRP admin command\n");
		epcmd->status = NVME_SC_DATA_XFER_ERROR | NVME_SC_DNR;
		goto complete;
	}
	...
	default:
		dev_err(&nvme->epf->dev, "Unhandled admin command 0x%02x\n",
			cmd->common.opcode);
	...
	if (transfer_len) {
		...
		ret = pci_epf_nvme_cmd_setup(nvme, epcmd, transfer_len);
		if (ret)
			goto complete;
	}

	pci_epf_nvme_exec_cmd(nvme, epcmd, async);

	if (post_process_hook)
		post_process_hook(nvme, epcmd);

	/* Command done: transfer data if needed */
	if (transfer_len) {
...
static bool pci_epf_nvme_process_io_cmd(struct pci_epf_nvme *nvme, int qid)
{
	...
	epcmd = pci_epf_nvme_fetch_command(nvme, qid);
	if (!epcmd)
		return false;
	...
	case nvme_cmd_dsm:
		/* Dataset Management is advisory only: do nothing */
		goto complete;

	default:
		dev_err(&nvme->epf->dev, "Unhandled IO command 0x%02x\n",
			(int)epcmd->cmd.common.opcode);
		epcmd->status = NVME_SC_INVALID_OPCODE | NVME_SC_DNR;
		goto complete;
	}

	if (dir != DMA_NONE) {
		size_t nrlbas = le16_to_cpu(epcmd->cmd.rw.length) + 1;

		ret = pci_epf_nvme_cmd_setup(nvme, epcmd,
					nrlbas << target->ns.blksize_shift);
		if (ret)
			goto complete;
...
static int pci_epf_nvme_alloc_reg_bar(struct pci_epf_nvme *nvme)
{
	...
	dev_dbg(&epf->dev,
		"Allocated register BAR %d, virt addr 0x%llx, phys addr 0x%llx, %zu B\n",
...
static int pci_epf_nvme_bind(struct pci_epf *epf)
{
	...
	if (nvme->dma_supported) {
		dev_info(&epf->dev, "DMA supported %s\n",
			 nvme->dma_enable ?
...
PCI_EPF_NVME_STR_RW(hostnqn)
CONFIGFS_ATTR(pci_epf_nvme_, hostnqn);

PCI_EPF_NVME_STR_RW(subsysnqn)
...
PCI_EPF_NVME_STR_RW(serial)
CONFIGFS_ATTR(pci_epf_nvme_, serial);

PCI_EPF_NVME_STR_RW(firmware_rev)
