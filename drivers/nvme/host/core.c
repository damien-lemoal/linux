...
static enum rq_end_io_ret nvme_async_cmd_endio(struct request *req,
					       blk_status_t blk_status)
{
	struct nvme_async_cmd_endio_cb *endio_cb = req->end_io_data;
	union nvme_result result;
	int status = 0;

	if (endio_cb && endio_cb->end_io) {
		...
		if (status >= 0)
			result = nvme_req(req)->result;
		endio_cb->end_io(endio_cb->data, status, &result);
	}
		...
		struct nvme_async_cmd_endio_cb *endio_cb)
{
	...
	req->end_io = nvme_async_cmd_endio;
	req->end_io_data = endio_cb;
