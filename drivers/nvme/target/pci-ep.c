...
#define NVMET_PCIEP_IO_SQ_POLL_LIMIT		msecs_to_jiffies(5)
...
static int nvmet_pciep_post_iod_cqe(struct nvmet_pciep_iod *iod)
{
	struct nvmet_pciep_ctrl *ctrl = iod->ctrl;
				 ...
				 u16 qsize, u64 pci_addr)
{
	...
	sq->iod_wq = alloc_workqueue("sq%d_wq", WQ_HIGHPRI | WQ_UNBOUND,
				   ...
				   unsigned int max_nr_queues)
{
	...
	ctrl->wq = alloc_workqueue("%s_ctrl_wq", WQ_UNBOUND,
...
static void nvmet_pciep_exec_iod(struct nvmet_pciep_iod *iod)
{
	...
	/*
	 * If we do not have data to transfer once the command completes,
...
static bool nvmet_pciep_init_iod(struct nvmet_pciep_iod *iod)
{
	struct nvmet_req *req = &iod->req;

	/*
	 * The command may or may not be using SGLs. However, the target
	 * controller mandates SGL use, which we set here, after remembering
	 * the command original flags.
	 */
	...
	iod->data_length = nvmet_req_transfer_len(req);
	if (iod->data_length) {
		/*
		 * Get the data DMA transfer direction. Here "device" is the
		 * PCI RC host.
		 */
...
static void nvmet_pciep_process_iod(struct nvmet_pciep_iod *iod)
{
	struct nvmet_pciep_queue *sq = iod->sq;

	dev_dbg(iod->ctrl->dev, "SQ[%u]: %s command %s (0x%02x)\n",
		sq->qid, sq->qid ? "Admin" : "IO",
		nvmet_pciep_iod_name(iod), iod->cmd.common.opcode);

	if (!nvmet_pciep_init_iod(iod)) {
		nvmet_pciep_complete_iod(iod);
		return;
	}

	/*
	 * If this is an admin command or if we do not have any remaining
	 * command to process, execute it directly in this context.
	 */
	if (!sq->qid || list_empty(&sq->list))
		nvmet_pciep_exec_iod(iod);
	else
		queue_work(sq->iod_wq, &iod->work);
}
				   ...
				   struct nvmet_pciep_queue *sq)
{
	...
	/*
	 * ...we do: map the queue and get them. If the mapping fails because of
	 * the lack of mapping windows, we will try again later.
	 */
...
static void nvmet_pciep_sq_work(struct work_struct *work)
{
	...
	while (test_bit(NVMET_PCIEP_Q_LIVE, &sq->flags)) {
		...
		/*
		 * We have no more command to execute: keep polling the SQ of
		 * IO queues for at most NVMET_PCIEP_IO_SQ_POLL_LIMIT and fall
		 * back to delay-rescheduling the SQ work if we have not
		 * received any command after that. This hybrid spin-polling
		 * method significantly increases the IOPS for shallow queue
		 * depth workloads (e.g. QD=1).
		 */
		if (!sq->qid)
			break;
		if (!end_poll)
			end_poll = jiffies + NVMET_PCIEP_IO_SQ_POLL_LIMIT;
		if (jiffies > end_poll)
			break;
...
static void nvmet_pciep_cq_work(struct work_struct *work)
{
	...
	while (!list_empty(&cq->list)) {
		...
		while (!list_empty(&list)) {
			iod = list_first_entry(&list, struct nvmet_pciep_iod, link);
			list_del_init(&iod->link);
			n += nvmet_pciep_post_iod_cqe(iod);
		}
...
static void nvmet_pciep_epf_epc_deinit(struct pci_epf *epf)
{
	...
	mutex_destroy(&nvme_epf->irq_lock);
}
