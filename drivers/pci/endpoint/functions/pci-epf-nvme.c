...
static int pci_epf_nvme_map(struct pci_epf_nvme *nvme, int slot,
			    u64 addr, size_t size)
{
...
static __always_inline int pci_epf_nvme_map_sgl(struct pci_epf_nvme *nvme,
...
static __always_inline int pci_epf_nvme_map_ksgl(struct pci_epf_nvme *nvme,
...
static __always_inline int pci_epf_nvme_map_prp(struct pci_epf_nvme *nvme,
						int slot, u64 prp)
{
	if (!prp)
		return 0;
...
static __always_inline int pci_epf_nvme_map_prp1(struct pci_epf_nvme *nvme,
						 struct nvme_command *cmd)
{
...
static __always_inline int pci_epf_nvme_map_prp2(struct pci_epf_nvme *nvme,
						 struct nvme_command *cmd)
{
...
static int pci_epf_nvme_map_prplist(struct pci_epf_nvme *nvme, int slot)
{
	...
	while (span) {
		if (pci_epf_nvme_map(nvme, slot, base, PRP_MAP_SIZE)) {
			dev_err(&nvme->epf->dev,
				"pci epf nvme: prplist map region error\n");
			return -ENOMEM;
		}
		...
		if (++slot == PRP_MAPS) {
			dev_err(&nvme->epf->dev,
				"pci epf nvme: prplist map out of resources\n");
			return -ENOMEM;
		}
...
static int pci_epf_nvmet_execute(struct pci_epf_nvme_target *target)
{
	struct nvmet_req *req = &target->req;
	struct pci_epf_nvme *nvme =
		container_of(target, struct pci_epf_nvme, target);

	dev_info(&nvme->epf->dev, "%s\n", __FUNCTION__);
...
static int pci_epf_nvmet_execute_sg_table(struct pci_epf_nvme_target *target,
...
static int pci_epf_nvmet_execute_buffer(struct pci_epf_nvme_target *target)
{
	struct nvmet_req *req = &target->req;
...
static int pci_epf_nvmet_execute_sgl_ksgl(struct pci_epf_nvme_target *target)
{
	struct nvmet_req *req = &target->req;
	struct scatterlist *sg = target->sgl;
...
static int pci_epf_nvmet_execute_prp1_prp2(struct pci_epf_nvme_target *target)
{
	struct nvmet_req *req = &target->req;
	struct scatterlist *sg = target->sgl;
	struct pci_epf_map *map = target->map;
...
static int pci_epf_nvmet_execute_prp1(struct pci_epf_nvme_target *target)
{
	struct nvmet_req *req = &target->req;
	struct scatterlist *sg = target->sgl;
	struct pci_epf_map *map = target->map;
...
static int pci_epf_nvmet_execute_prplist(struct pci_epf_nvme_target *target,
...
static int pci_epf_nvmet_execute_prp1list(struct pci_epf_nvme_target *target)
{
	struct pci_epf_nvme *nvme =
		container_of(target, struct pci_epf_nvme, target);
	...
	return pci_epf_nvmet_execute_prplist(target, 0,
					     target->req.transfer_len);
}

static int pci_epf_nvmet_execute_request(struct pci_epf_nvme_target *target,
					 size_t transfer_len, int qid)
{
	struct pci_epf_nvme *nvme =
		container_of(target, struct pci_epf_nvme, target);
	struct pci_epf_map *map = target->map;
...
out:
	dev_info(&nvme->epf->dev, "req exec status %d\n", status);
				 ...
				 u16 qid, u16 qsize, u16 command_id)
{
	...
	return pci_epf_nvmet_execute_request(target, sizeof(*data), qid);
}

static int pci_epf_nvmet_connect_admin_queue(struct pci_epf_nvme_target *target)
{
	struct pci_epf_nvme *nvme =
		container_of(target, struct pci_epf_nvme, target);
	int ret;

	dev_info(&nvme->epf->dev, "Connect admin queue\n");

	ret = pci_epf_nvmet_connect(target, 0, (NVME_AQ_DEPTH - 1), 0);
	if (ret)
		return ret;
...
static void pci_epf_nvme_target_keep_alive(struct pci_epf_nvme *nvme)
{
	...
	pci_epf_nvmet_execute_request(&nvme->target, 0, 0);
}
...
static void pci_epf_nvme_target_stop(struct pci_epf_nvme *nvme)
{
	...
	dev_info(&nvme->epf->dev, "Stop target\n");
...
static int pci_epf_nvme_target_start(struct pci_epf_nvme *nvme)
{
	...
	dev_info(&nvme->epf->dev, "Start target\n");

	for (qid = 0; qid <= PCI_EPF_NVME_QIDMAX; qid++)
		nvmet_sq_init(sq++);

	ret = pci_epf_nvmet_connect_admin_queue(target);
	if (ret) {
		dev_err(&nvme->epf->dev,
			"Failed to connect target admin queue\n");
		return ret;
	}
...
static void pci_epf_nvme_target_init(struct pci_epf_nvme *nvme)
{
	...
	dev_info(&nvme->epf->dev, "Init target\n");
...
static void pci_epf_nvme_host_init(struct pci_epf_nvme *nvme)
{
	...
	dev_info(&nvme->epf->dev, "Init host\n");
...
static int pci_epf_nvme_admin_identify(struct pci_epf_nvme *nvme)
{
	...
	case NVME_ID_CNS_CTRL:
		if (pci_epf_nvme_map_prp1(nvme, cmd)) {
			...
			status = pci_epf_nvmet_execute_request(target, 0, 0);
			if (req->cqe->status)
...
static void pci_epf_nvme_admin_cmd_handler(struct pci_epf_nvme *nvme)
{
	struct pci_epf_nvme_target *target = &nvme->target;
	struct nvmet_req *req = &target->req;
	struct nvme_command *cmd = req->cmd;
	...
	else if (pci_epf_nvmet_execute_request(target, 0, qid))
...
static void pci_epf_nvme_io_cmd_handler(struct pci_epf_nvme *nvme)
{
	struct pci_epf_nvme_target *target = &nvme->target;
	struct nvmet_req *req = &target->req;
	struct nvme_command *cmd = req->cmd;
...
static void pci_epf_nvme_cmd_handler(struct work_struct *work)
{
	...
	if (host->cc & NVME_CC_ENABLE) {
		...
		pci_epf_nvme_admin_cmd_handler(nvme);
		pci_epf_nvme_io_cmd_handler(nvme);
...
static int pci_epf_nvme_alloc_space(struct pci_epf *epf)
{
	...
	dev_info(&epf->dev, "Allocating space\n");
	...
	for (bar = BAR_0; bar < PCI_STD_NUM_BARS; bar += add) {
		...
		dev_info(&epf->dev, "Allocating BAR %d (%zu B)\n",
			 bar, bar_size[bar]);

		base = pci_epf_alloc_space(epf, bar_size[bar], bar, align,
					   PRIMARY_INTERFACE);
		if (!base)
			dev_err(dev, "Allocate BAR%d space failed\n", bar);
		nvme->reg[bar] = base;
	}
...
static int pci_epf_nvme_core_init(struct pci_epf *epf)
{
	...
	if (epc_features->msi_capable) {
		dev_info(&epf->dev, "MSI capable, %d interrupts\n",
			 epf->msi_interrupts);
	...
	if (epc_features->msix_capable) {
		dev_info(&epf->dev, "MSIX capable, %d interrupts\n",
			 epf->msix_interrupts);
...
static int pci_epf_nvme_bind(struct pci_epf *epf)
{
	...
	if (!nvme->ns_device_path) {
		dev_err(&epf->dev, "NS device path not set\n");
		return -EINVAL;
	}

	dev_info(&epf->dev, "Bind device %s\n", nvme->ns_device_path);
...
static int pci_epf_nvme_probe(struct pci_epf *epf)
{
	...
	dev_info(&epf->dev, "Probe func %d, vfunc %d\n",
