...
#define pci_epf_nvme_prp_size(prp)	\
	(NVME_CTRL_PAGE_SIZE - pci_epf_nvme_prp_ofst(prp))
...
struct pci_epf_nvme_map {
	...
	/* Root port PCI space address to map */
	phys_addr_t	pci_base;
	phys_addr_t	pci_addr;

	/* Local physical address of mapped PCI address */
	phys_addr_t	phys_base;
	phys_addr_t	phys_addr;

	/* Local virtual address of mapped PCI address */
...
/*
 * Map @size bytes from @pci_addr in the host PCI space into local memory.
 * The locally mapped memory is indicated by map->virt_addr.
 */
			    ...
			    phys_addr_t pci_addr, size_t size)
{
	...
	align = nvme->epc_features->align;
	if (!align)
		align = PAGE_SIZE;

	map->pci_addr = pci_addr;
	map->pci_base = ALIGN_DOWN(pci_addr, align);
	ofst = pci_addr - map->pci_base;
	map->size = ALIGN(size + ofst, align);
	...
	dev_dbg(&nvme->epf->dev,
		"Map PCI addr 0x%llx/%zu B -> phys 0x%llx, virt 0x%llx, %zu B\n",
		map->pci_addr, size, map->phys_addr,
		(unsigned long long)map->virt_addr, map->size);

	return 0;

free:
	pci_epc_mem_free_addr(epc, map->phys_base, map->virt_base, map->size);
err:
	map->virt_base = NULL;
	map->phys_base = 0;
	map->size = 0;
				 ...
				 void *buf)
{
	...
	/* Map segment */
	ret = pci_epf_nvme_map(nvme, &map, seg->pci_addr, seg->size);
	if (ret)
		return ret;
				     ...
				     size_t transfer_len)
{
	...
	prp_size = min_t(size_t, pci_epf_nvme_prp_size(prp),
			 transfer_len >> (NVME_CTRL_PAGE_SHIFT - 3));
	ret = pci_epf_nvme_map(nvme, &map, prp, prp_size);
	if (ret)
		return ret;
					     ...
					     size_t transfer_len)
{
	...
	prp_size = pci_epf_nvme_prp_size(prp1);
	if (transfer_len > prp_size) {
	...
	if (nr_segs == 1) {
		...
		epcmd->segs[0].size = prp_size;
		epcmd->segs[1].pci_addr = prp2;
		epcmd->segs[1].size = transfer_len - prp_size;
	}
					      ...
					      int qid)
{
	...
	ret = pci_epf_nvme_map(nvme, &sq->map, sq->pci_addr, qsize);
	if (ret) {
	...
	ret = pci_epf_nvme_map(nvme, &cq->map, cq->pci_addr, qsize);
	if (ret) {
...
static ssize_t pci_epf_nvme_ns_device_path_show(struct config_item *item,
						char *page)
{
...
static ssize_t pci_epf_nvme_ns_device_path_store(struct config_item *item,
						 const char *page, size_t len)
{
	...
	if (!slen || slen > sizeof(nvme->ns_device_path) - 1)
		return -EINVAL;

	strncpy(nvme->ns_device_path, page, slen);
