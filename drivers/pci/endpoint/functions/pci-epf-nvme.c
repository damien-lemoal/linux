...
/*
 * Maximum number of IO queue pairs (1 + max IO queues).
 */
#define PCI_EPF_NVME_MAX_NR_QUEUES	(1 + 4)
...
generic:
	...
	dev_info(dev, "Generic DMA channel %s: maximum segment size %u B\n",
		 dma_chan_name(chan),
				     ...
				     void *buf, enum dma_data_direction dir)
{
	...
	dma_phys_addr = dma_map_single(epf_nvme->dma_dev, buf,
				       map->pci_size, dir);
	if (dma_mapping_error(epf_nvme->dma_dev, dma_phys_addr)) {
		dev_err(dev, "Map buffer for DMA failed\n");
		return -ENOMEM;
	}
	...
	if (epf_nvme->dma_private) {
		sconf.direction = xfer_dir;
		if (xfer_dir == DMA_MEM_TO_DEV)
			sconf.dst_addr = map->pci_addr;
		else
			sconf.src_addr = map->pci_addr;
		...
		tx = dmaengine_prep_slave_single(chan, dma_phys_addr,
					map->pci_size, xfer_dir,
...
unmap:
	dma_unmap_single(epf_nvme->dma_dev, dma_phys_addr, map->pci_size, dir);
...
{
	struct pci_epf *epf = epf_nvme->epf;
	phys_addr_t pci_addr = seg->pci_addr;
	...
	while (size) {
		...
		/* Map segment */
		map_size = pci_epf_mem_map(epf, pci_addr, size, &map);
		if (map_size < 0)
			return map_size;
		...
		if (!epf_nvme->dma_enable || map_size < SZ_4K)
...
static int pci_epf_nvme_fetch_cmd(struct pci_epf_nvme *epf_nvme,
				  struct pci_epf_nvme_queue *sq)
{
	struct pci_epf_nvme_ctrl *ctrl = &epf_nvme->ctrl;
	struct pci_epf_nvme_cmd *epcmd;
	...
	while (sq->state == PCI_EPF_NVME_QUEUE_LIVE) {
...
static bool pci_epf_nvme_queue_response(struct pci_epf_nvme_cmd *epcmd)
{
	...
	if (!pci_epf_nvme_ctrl_ready(ctrl) ||
		...
		dev_dbg(&epf_nvme->epf->dev, "cq[%d]: queue not live\n",
			epcmd->cqid);
		goto free_cmd;
	}
...
static void pci_epf_nvme_drain_sq(struct pci_epf_nvme *epf_nvme, int qid)
{
	...
	while (pci_epf_nvme_fetch_cmd(epf_nvme, sq)) {
...
static void pci_epf_nvme_disable_ctrl(struct pci_epf_nvme *epf_nvme)
{
	 ...
	 * on the completion queues. This also stops polling for
...
static void pci_epf_nvme_delete_ctrl(struct pci_epf *epf)
{
	...
	dev_info(&epf->dev, "Deleting fabrics host controller\n");
...
static int pci_epf_nvme_create_ctrl(struct pci_epf *epf)
{
	...
	dev_info(&epf->dev, "Creating fabrics host controller\n");
	...
	fctrl = nvmf_create_ctrl(dev, epf_nvme->ctrl_opts_buf);
	if (IS_ERR(fctrl)) {
	...
	/* Allocate queues */
	ctrl->nr_queues = fctrl->queue_count;
	if (ctrl->nr_queues > epf_nvme->max_nr_queues) {
		dev_err(dev, "Too many queues (%u > maximum allowed %u)\n",
			ctrl->nr_queues, epf_nvme->max_nr_queues);
		ret = -EINVAL;
		goto out_delete_ctrl;
	}
...
static void pci_epf_nvme_sq_work(struct work_struct *work)
{
	...
	while (pci_epf_nvme_ctrl_ready(ctrl) &&
	       pci_epf_nvme_fetch_cmd(epf_nvme, sq)) {
...
static int pci_epf_nvme_alloc_reg_bar(struct pci_epf *epf)
{
	 ...
	 * limits.
	 */
	dev_info(&epf->dev, "Controller has %u memory windows\n",
		 epf->epc->num_windows);

	if (epf->epc->num_windows > 1)
		epf_nvme->max_nr_queues = epf->epc->num_windows / 3;
	else
		epf_nvme->max_nr_queues = PCI_EPF_NVME_MAX_NR_QUEUES;

	if (features->msix_capable && epf->msix_interrupts) {
		dev_info(&epf->dev, "Controller supports MSIX, %u vectors\n",
			 epf->msix_interrupts);
		epf_nvme->max_nr_queues =
			min_t(unsigned int, epf->msix_interrupts,
			      epf_nvme->max_nr_queues);
	} else if (features->msi_capable && epf->msi_interrupts) {
		dev_info(&epf->dev, "Controller supports MSI, %u vectors\n",
			 epf->msi_interrupts);
		epf_nvme->max_nr_queues =
	...
	if (epf_nvme->max_nr_queues <= 1) {
		dev_err(&epf->dev, "Invalid maximum number of queues %u\n",
			epf_nvme->max_nr_queues);
		return -EINVAL;
	}

	dev_info(&epf->dev, "Maximum number of I/O queues: %u\n",
