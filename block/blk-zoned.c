...
static void blk_zone_wplug_bio_work(struct work_struct *work)
{
	...
	/*
	 * blk-mq reuses the reference count on the disk request queue
	 * we got when plugging the BIO. But BIO-based devices do not do
	 * that. So drop this extra reference once we submit the BIO.
	 */
	if (bio->bi_bdev->bd_has_submit_bio)
