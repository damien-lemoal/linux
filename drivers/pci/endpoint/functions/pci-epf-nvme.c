				   ...
				   struct pci_epf_nvme_queue *cq)
{
	...
	case PCI_EPC_IRQ_MSI:
		ret = pci_epc_raise_irq(epf->epc, epf->func_no, epf->vfunc_no,
					nvme->irq_type, cq->vector + 1);
		if (!ret)
			return;
		/*
		 * If this fails, it is likely because the host is using
		 * legacy IRQs (e.g. BIOS), so fallthrough.
		 */
		fallthrough;
	...
	if (ret)
		dev_err(&epf->dev, "Raise IRQ %d failed %d\n", irq, ret);
}
...
static void pci_epf_nvme_drain_sq(struct pci_epf_nvme *nvme, int qid)
{
...
static void pci_epf_nvme_ctrl_disable(struct pci_epf_nvme *nvme)
{
	...
	for (qid = PCI_EPF_NVME_MAX_QID; qid >= 0; qid--)
		pci_epf_nvme_drain_sq(nvme, qid);
...
static int pci_epf_nvme_init_irq(struct pci_epf_nvme *nvme)
{
	...
	if (nvme->epc_features->msix_capable) {
		...
		nvme->irq_type = PCI_EPC_IRQ_MSIX;
	...
	/* Legacy INTX */
	nvme->nr_vectors = 1;
	nvme->irq_type = PCI_EPC_IRQ_LEGACY;
