 ...
 */
#define PCI_EPF_NVME_MAX_NR_QUEUES	(1 + 8)
...
/*
 * Maximum data transfer size: limit to 512 KB to avoid excessive local
...
static int pci_epf_nvme_cmd_parse_dptr(struct pci_epf_nvme_cmd *epcmd)
{
	...
	if (epcmd->buffer_size > PCI_EPF_NVME_MDTS)
		goto invalid_field;
...
static void pci_epf_nvme_async_cmd_done_cb(void *data, int status,
...
static bool pci_epf_nvme_queue_response(struct pci_epf_nvme_cmd *epcmd)
{
	...
	if (!pci_epf_nvme_ctrl_ready(ctrl) ||
		...
		dev_err(&epf_nvme->epf->dev, "cq[%d]: queue not live\n",
			epcmd->cqid);
		goto free_cmd;
	}
					   ...
					   struct pci_epf_nvme_cmd *epcmd)
{
	...
	case nvme_admin_async_event:
		epcmd->cmd_cb.complete = pci_epf_nvme_async_cmd_done_cb;
		ret = nvme_submit_async_cmd(epf_nvme->ctrl.ctrl->admin_q, cmd,
					    NULL, 0, &epcmd->cmd_cb);
		if (ret) {
			epcmd->status = NVME_SC_INTERNAL | NVME_SC_DNR;
			goto complete;
					...
					struct pci_epf_nvme_cmd *epcmd)
{
	...
	queue_work(system_unbound_wq, &epcmd->io_work);
