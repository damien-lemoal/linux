...
static void k210_clk_set_divider(struct device_node *np,
				 int id, unsigned long divider)
{
	struct k210_clk_cfg *kclk = &k210_clks[id];
	u32 reg, old_div, new_div, div_mask;

	if (!divider) {
		pr_err("%pOFP: clock %d: invalid zero divider\n", np, id);
		return;
	}
	...
	case DIV_DOUBLE_ONE_BASED:
		if (divider & 1) {
			pr_err("%pOFP: clock %d: divider must be an even value\n",
			       np, id);
			return;
		}
		new_div = (divider >> 1) - 1;
		break;
	case DIV_POWER_OF_TWO:
		if (!is_power_of_2(divider)) {
			pr_err("%pOFP: clock %d: divider must be a power of 2\n",
			       np, id);
			return;
		}
		new_div = ilog2(divider);
		break;
	case DIV_NONE:
	default:
		pr_err("%pOFP: clock %d: not a divider clock\n", np, id);
		return;
	}

	if (new_div >= (1 << kclk->div_width)) {
		pr_err("%pOFP: clock %d: divider value too large\n", np, id);
		return;
	}

	old_div = k210_clk_get_div_val(kclk);
	if (old_div != new_div) {
		pr_info("%pOFP: clock %d: changing divider from %lu to %lu\n",
			np, id,
			k210_clk_divider(kclk, old_div),
			k210_clk_divider(kclk, new_div));

		reg = readl(kcl->regs + kclk->div_reg);
		div_mask = GENMASK(kclk->div_width - 1, 0);
		reg &= ~(div_mask << kclk->div_shift);
		reg |= (new_div & div_mask) << kclk->div_shift;
		writel(reg, kcl->regs + kclk->div_reg);
	}
}
				       ...
				       unsigned long parent_rate)
{
	...
	if (!divider) {
		pr_err("Invalid zero divider for clock %u\n",
		       to_k210_clk_id(hw));
		return 0;
	}
...
static void k210_clk_set_dividers(struct device_node *np)
{
	struct k210_clk_cfg *kclk;
	struct property *prop;
	const __be32 *p;
	u32 id, divider;

	/*
	 * property k210,clock-dividers: <clkid divider>, [<..>]
	 * allow to specify divider values for all clocks that have one.
	 */
	of_property_for_each_u32(np, "k210,clock-dividers", prop, p, id) {

		if (id >= K210_NUM_CLKS) {
			pr_err("%pOFP: invalid clock ID %u\n", np, id);
			return;
		}
		kclk = &k210_clks[id];

		p = of_prop_next_u32(prop, p, &divider);
		if (!p) {
			pr_err("%pOFP: missing divider value for clock %d\n",
			       np, id);
			return;
		}

		k210_clk_set_divider(np, id, divider);
	}
}
...
static void __init k210_clk_init(struct device_node *np)
{
	...
	if (!kcl) {
		pr_err("%pOFP: data allocation failed\n", np);
		return;
	}

	sysctl_np = of_find_compatible_node(NULL, NULL, "kendryte,k210-sysctl");
	if (!sysctl_np) {
		pr_err("%pOFP: sysctl node not found\n", np);
		goto err;
	}

	kcl->regs = of_iomap(sysctl_np, 0);
	if (!kcl->regs) {
		pr_err("%pOFP: map sysctl resources failed\n", np);
		goto err;
	}
	...
	kcl->clk_data = kzalloc(struct_size(kcl->clk_data, hws, K210_NUM_CLKS),
				GFP_KERNEL);
	if (!kcl->clk_data) {
		pr_err("%pOFP: clk data allocation failed\n", np);
		goto err;
	}
	 ...
	 * used as general memory, make it critical.
	 */
	 hws[K210_CLK_AI] =
