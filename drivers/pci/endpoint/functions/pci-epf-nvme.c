...
/*
 * Define host (root complex side) PCI address mapping to our
 * local EP local memory.
 */
struct pci_epf_nvme_map {

	/* Root complex PCI address to map */
			    ...
			    phys_addr_t pci_addr, size_t size)
{
	...
	/*
	 * The endpoint controller will use the lower bits of the local
	 * PCI address to drive the PCI transfers to the host. Determine
	 * the address mask we need to care about based on the host PCI address
	 * and the trnasfer size.
	 */
	sz = 1UL << fls64(pci_addr ^ (pci_addr + size - 1));
	mask = (1UL << ilog2(sz)) - 1;
	...
	/* Map the local memory to the host PCI address */
	ret = pci_epc_map_addr(epf->epc, epf->func_no, epf->vfunc_no,
			       map->phys_addr, map->pci_addr, size);
	if (ret) {
	...
	dev_dbg(&nvme->epf->dev,
		"Map PCI addr 0x%llx + %zu B -> phys 0x%llx, virt 0x%lx, ofst 0x%lx, %zu B\n",
		map->pci_addr, size, map->phys_base,
		(unsigned long)map->virt_base, ofst, map->size);
				     ...
				     size_t mapping_size)
{
	...
	dev_dbg(&nvme->epf->dev, "PRP list: prp 0x%llx / %zu B / %zu B\n",
		prp, pci_epf_nvme_prp_size(prp),
					   ...
					   size_t transfer_len)
{
	...
	dev_dbg(&nvme->epf->dev, "PRP list: prp1 0x%llx + %zu B\n",
		prp, seg->size);
	...
	while (size < transfer_len) {
		...
		dev_dbg(&nvme->epf->dev,
			"PRP list: prp %d 0x%llx, %zu B / %zuB\n",
			i, prp, size, transfer_len);
	...
	dev_dbg(&nvme->epf->dev, "PRP list: %d segments\n", nr_segs);
					     ...
					     size_t transfer_len)
{
	...
	if (nr_segs == 1) {
		...
		dev_dbg(&nvme->epf->dev, "PRPs: 1 segment at 0x%llx + %zu B\n",
			prp1, transfer_len);
	} else {
		...
		dev_dbg(&nvme->epf->dev,
			"PRPs: 2 segments at 0x%llx + %zu B, 0x%llx + %zu B\n",
			prp1, prp1_size, prp2, epcmd->segs[1].size);
	}
				       ...
				       size_t transfer_len)
{
	...
	dev_dbg(&nvme->epf->dev, "Parse dptr, transfer length %zu B\n",
		transfer_len);
